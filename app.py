...
# Free/open data + Groq Compound web research + Streamlit
# ------------------------------------------------------------
...
def groq_client():
    ...
    if not key:
        return None
    return Groq(api_key=key, default_headers={"Groq-Model-Version": "latest"})


def research_with_groq(trip, weather_summary):
    """
    Uses Groq Compound for current web research.
    The model can search the web and visit public websites.
    """
    client = groq_client()
    if not client:
...
SOURCE PRIORITY:
- Weather cross-check: weather.com when useful, but use the supplied Open-Meteo forecast as
  the primary structured weather dataset.
- Attractions/reviews: Tripadvisor, GetYourGuide and reputable local tourism sources.
- Hotels: Tripadvisor, Wego, Booking.com, Agoda, Google travel/search snippets, and reputable
  local platforms where available.
- Local issues: official government/tourism/road authorities and reputable news.
- Use online user feedback as a secondary signal, not as established fact.
...
Include clickable URLs in the Sources section when you can identify them.
"""
    ...
    try:
        ...
        return (
            ...
            "Please check the Groq API key, model access, and Streamlit logs. "
...
st.markdown(
    ...
    <div class="hero">
        <h1>🧭 {APP_NAME}</h1>
        <p>AI-assisted route planning, weather-aware travel advice, sights, stays and family guidance.</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
if generate:
    ...
    with st.status("Building your travel plan...", expanded=True) as status:
        ...
        st.write("4. Asking Groq Compound to research current travel information...")
        trip = {
            "points": points,
...
if "trip" not in st.session_state:
    ...
    st.markdown(
        ...
        - 🤖 Uses Groq Compound for live web research.
...
st.subheader("🤖 AI travel research")
